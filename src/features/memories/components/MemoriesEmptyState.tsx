    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.title}>
                {isNoPartner
                    ? t("memories.emptyNoPartnerTitle")
                    : t("memories.emptyNoMemoriesTitle")}
            </Text>
            <Text style={styles.subtitle}>
                {isNoPartner
                    ? t("memories.emptyNoPartnerSubtitle")
                    : t("memories.emptyNoMemoriesSubtitle")}
            </Text>

            {!isNoPartner && onAddMemory && (
                <TouchableOpacity style={styles.btn} onPress={onAddMemory} activeOpacity={0.85}>
                    <Ionicons name="add-circle-outline" size={18} color="#fff" style={{ marginRight: 6 }} />
                    <Text style={styles.btnText}>{t("memories.emptyAddFirstMemory")}</Text>
                </TouchableOpacity>
            )}
        </View>
    );
